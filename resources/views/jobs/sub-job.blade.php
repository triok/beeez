@php($sub = isset($sub_id) ? $sub_id : 1)

<div class="sub-item">
    <table class="table" id="sub-{{$sub}}-task">
        <caption>
            <h4 class="popover-title cursor-pointer" id="sub-{{$sub}}-title">
                Task # {{$sub}}

                <span id="sub-{{$sub}}-num"></span>

                @if($sub != 1)
                    <span class="pull-right rm-sb" style="padding: 0 5px;">
        ...
        <tr class="form-group">
            ...
            <td>
                <input type="text" class="form-control" id="sub-{{$sub}}-name" name="sub-{{$sub}}-name">
            </td>
        </tr>
        <tr class="form-group">
            <td><label for="sub-{{$sub}}-desription">Description:</label></td>
            <td>
                <textarea class="form-control editor1" rows="3" id="sub-{{$sub}}-desription"
                          name="sub-{{$sub}}-desription"></textarea>
            </td>
        </tr>
        <tr class="form-group">
            <td><label for="sub-{{$sub}}-instruction">Instruction:</label></td>
            <td>
                <textarea class="form-control editor2" rows="3" id="sub-instruction-{{$sub}}"
                          name="sub-{{$sub}}-instruction"></textarea>
            </td>
        </tr>
        <tr class="form-group">
        ...
        <tr class="form-group">
            ...
            <td>
                @foreach($_skills as $skill)
                    <label class="checkbox-inline"><input type="checkbox" name="sub-{{$sub}}-skills[]"
                                                          id="sub-skill-{{$skill->id}}"
                                                          value="{{$skill->id}}">{{ucwords($skill->name)}}</label>
                @endforeach
            </td>
        </tr>
        <tr class="form-group">
            ...
            <td>
                @foreach($_categories as $category)
                    <label class="checkbox-inline"><input type="checkbox" name="sub-{{$sub}}-categories[]"
                                                          id="sub-category-{{$category->id}}"
                                                          value="{{$category->id}}">{{ucwords($category->nameEu)}}
                    </label>
                @endforeach
            </td>
        </tr>
